...
public class ClienteController implements Serializable {
    ...
    public Cliente getSelected() {
        if (current == null) {
            current = new Cliente();
            current.setClientePK(new entidade.ClientePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public static class ClienteControllerConverter implements Converter {
        ...
        entidade.ClientePK getKey(String value) {
            entidade.ClientePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new entidade.ClientePK();
            key.setIdCliente(Integer.parseInt(values[0]));
            key.setIdTipo(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(entidade.ClientePK value) {
